...
export const download = async (filename) => {
    const {data} = await $authHost.post(`api/files/${filename}`)
    return data
}
...
export const adminAddNote = async (selectedTable, formData) => {
    const {data} = await $authHost.post(`api/admin-add-note-bd`, {selectedTable, formData})
    return data
}